(() => {
  ...
  async function openOriginal(id) {
    ...
    const email = state.emails.find(item => item.id === id); if (!email) return;
    if (state.isDemo) { notify("Demo modunda bu i�lem yap�lamaz.", "uyari"); return; }
  ...
  async function aiReply(id, button) {
    const email = state.emails.find(item => item.id === id); if (!email) return;
    if (state.isDemo) { notify("Demo modunda yapay zeka yan�t� kullan�lamaz.", "uyari"); return; }
    ...
    try {
      ...
      window.open(url, "_blank", "noopener,noreferrer");
      notify("Yapay zeka yan�t� taslak olarak a��ld�.", "basari");
    } catch (_) { notify("Yapay zeka yan�t� �retilemedi.", "uyari"); }
  ...
  function copySummary(id) {
    ...
    navigator.clipboard.writeText(text).then(() => notify("�zet kopyaland�.", "basari")).catch(() => notify("Kopyalanamad�.", "uyari"));
  }
  ...
  function randomizeSlogan() {
    const slogans = [
      "Bug�n ger�ekten <em>�nemli</em> olanlar.",
      "Gelen kutunuzda kontrol <em>sizde</em>.",
      "Sadece <em>odaklanman�z</em> gerekenler.",
      "Zaman�n�z� <em>geri</em> kazan�n."
    ];
